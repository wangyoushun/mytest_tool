...
/**
 * 简单工厂模式 又称 静态工厂方法模式 简单工厂模式是由一个工厂对象决定创建出哪一种产品类的实例
 * 
 * dateFormate
 * @author 有顺
 * 
...
public class Main {
	...
	public static void main(String[] args) throws BadException {
		Shape factory = ShapeFactory.factory("square");
		factory.draw();
		factory.eraser();
	}
}
 ...
 * 当系统中的具体产品类不断增多时候
 * ，可能会出现要求工厂类根据不同条件创建不同实例的需求．这种对条件的判断和对具体产品类型的判断交错在一起，很难避免模块功能的蔓延
 * ，对系统的维护和扩展非常不利； 这些缺点在工厂方法模式中得到了一定的克服
 */
